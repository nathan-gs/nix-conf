...
let
  cfg = config.services.home-assistant-dev;
...
in {
  imports = [
    ...
    (mkRemovedOptionModule [ "services" "home-assistant-dev" "applyDefaultConfig" ] "The default config was migrated into services.home-assistant.config")
    (mkRemovedOptionModule [ "services" "home-assistant-dev" "autoExtraComponents" ] "Components are now parsed from services.home-assistant.config unconditionally")
    (mkRenamedOptionModule [ "services" "home-assistant-dev" "port" ] [ "services" "home-assistant" "config" "http" "server_port" ])
  ];
  ...
  options.services.home-assistant-dev = {
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc = lib.mkMerge [
      (lib.mkIf (cfg.config != null && !cfg.configWritable) {
        "home-assistant-dev/configuration.yaml".source = configFile;
      })

      (lib.mkIf (cfg.lovelaceConfig != null && !cfg.lovelaceConfigWritable) {
        "home-assistant-dev/ui-lovelace.yaml".source = lovelaceConfigFile;
      })
    ];

    systemd.services.home-assistant-dev = {
      ...
      preStart = let
        ...
        '' else ''
          ...
          ln -s /etc/home-assistant-dev/configuration.yaml "${cfg.configDir}/configuration.yaml"
        '';
        copyLovelaceConfig = if cfg.lovelaceConfigWritable then ''
          rm -f "${cfg.configDir}/ui-lovelace.yaml"
          cp --no-preserve=mode ${lovelaceConfigFile} "${cfg.configDir}/ui-lovelace.yaml"
        '' else ''
          ln -fs /etc/home-assistant-dev/ui-lovelace.yaml "${cfg.configDir}/ui-lovelace.yaml"
        '';
    ...
    systemd.targets.home-assistant-dev = rec {
      description = "Home Assistant";
      wantedBy = [ "multi-user.target" ];
      wants = [ "home-assistant-dev.service" ];
      after = wants;
    };
