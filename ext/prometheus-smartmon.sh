#!/bin/bash
#
# Derivation of https://github.com/prometheus-community/node-exporter-textfile-collector-scripts
# Changes:
#  - removed absolute paths, to work with NIX
#  - use /dev/disk/by-id names instead of /dev/sdX
#
#
...
for device in ${device_list}; do
  ...
  relative_disk="$(echo "$disk" | sed 's/\/dev/..\/../')"
  disk_byid="$(ls -l /dev/disk/by-id/ | grep -v wwn | grep -v nvme-nvme | grep -v '\-part' | grep "$relative_disk" | head -n1  | awk '{print $9}')"
#  echo "TEST $disk $type $relative_disk $disk_byid"
  active=1
  echo "smartctl_run{disk=\"${disk_byid}\",type=\"${type}\"}" "$(TZ=UTC date '+%s')"
  # Check if the device is in a low-power mode
  smartctl -n standby -d "${type}" "${disk}" > /dev/null || active=0
  echo "device_active{disk=\"${disk_byid}\",type=\"${type}\"}" "${active}"
  # Skip further metrics to prevent the disk from spinning up
  test ${active} -eq 0 && continue
  # Get the SMART information and health
  smartctl -i -H -d "${type}" "${disk}" | parse_smartctl_info "${disk_byid}" "${type}"
  # Get the SMART attributes
  case ${type} in
  sat) smartctl -A -d "${type}" "${disk}" | parse_smartctl_attributes "${disk_byid}" "${type}" ;;
  sat+megaraid*) smartctl -A -d "${type}" "${disk}" | parse_smartctl_attributes "${disk_byid}" "${type}" ;;
  scsi) smartctl -A -d "${type}" "${disk}" | parse_smartctl_scsi_attributes "${disk_byid}" "${type}" ;;
  megaraid*) smartctl -A -d "${type}" "${disk}" | parse_smartctl_scsi_attributes "${disk_byid}" "${type}" ;;
  nvme*) smartctl -A -d "${type}" "${disk}" | parse_smartctl_scsi_attributes "${disk_byid}" "${type}" ;;
  *)
